<div class="tab-pane fade" id="mmaintenance_mode_tab" role="tabpanel">
    ...
    <form action="{{ route('admin.update-maintenance-mode') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group ">
            <label>{{ __('Maintenance Mode Image') }}<span class="text-danger"></span></label>
            <div id="image-preview-maintenance" class="image-preview"
                @if (!empty($setting?->maintenance_image)) style="background-image: url({{ asset($setting?->maintenance_image) }}); background-size: cover; background-position: center center;" @endif>
        ...
        <div class="form-group">
            ...
            <textarea name="maintenance_description" id="" cols="30" rows="10" class="summernote" data-translate="true">{!! clean($setting->maintenance_description) !!}</textarea>
        </div>
