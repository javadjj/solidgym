    @if ($razorpay_credentials->razorpay_status == 'active')
        ...
        <form action="{{ route('wallet.pay-via-razorpay') }}" method="POST" class="d-none">
            @csrf
    ...
    @if ($mollie_credentials->mollie_status == 'active')
        <a href="{{ route('wallet.pay-via-mollie', ['payable_amount' => $payable_amount]) }}" class="btn btn-primary">
            Pay with Mollie
        </a>
    @endif

    @if ($instamojo_credentials->instamojo_status == 'active')
        <a href="{{ route('wallet.pay-via-instamojo', ['payable_amount' => $payable_amount]) }}"
                    ...
                    callback: function(data) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                window.location.href = "{{ route('wallet.payment-addon-success') }}";
                            },
                            error: function(err) {
