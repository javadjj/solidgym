    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>{{ __('Counter List') }}</h1>
                <div class="section-header-breadcrumb">
                    <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a>
                    </div>
                    <div class="breadcrumb-item">{{ __('Counter List') }}</div>
                </div>
            </div>
                    ...
                    <div class="col">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <x-admin.form-title :text="__('Counter List')" />
                                <div>
                                    @if ($counters->count() < 4)
                                        <x-admin.add-button :href="route('admin.counter.create')" text="{{ __('Add Counter') }}" />
                                    @endif
                                </div>
                            </div>
                            <div class="card-body">
                                        ...
                                        <tbody>
                                            @forelse ($counters as $index => $counter)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="btn-group">
                                                            <a href="{{ route('admin.counter.edit', $counter->id) }}?code={{ getSessionLanguage() }}"
                                            ...
                                            @empty
                                                <x-empty-table :name="__('Counter')" route='admin.partner.create' create="yes"
